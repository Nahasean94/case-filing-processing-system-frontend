...
import {findCourtPendingCases} from "../../../shared/queries"
    ...
    componentDidMount() {
        ...
        this.props.graphql
            .query({
                ...
                operation: {
                    variables: {
                        court_station: token.court_station,
                    },
                    query: findCourtPendingCases
                }
            })
            .request.then(({data, loading, error}) => {
                if (data) {
                    if (data.findCourtPendingCases) {
                        this.setState({cases: data.findCourtPendingCases, loading: false, error: false})
                    }
                }
                if (loading) {
